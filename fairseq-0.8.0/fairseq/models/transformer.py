...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if self.add_template:
            ...
            self.positionwise = PositionWise(embed_dim, embed_dim, self.dropout)
        
        
            self.two_encoder_mix = nn.Linear(2 * embed_dim, embed_dim)

            self.attention = MultiheadAttention(embed_dim, args.encoder_attention_heads,
    ...
    def forward(self, src_tokens, src_lengths, template = None):
        ...
        if self.add_template:
            ...
            adj_att, _ = self.attention(query=x, key=tp, value=tp, key_padding_mask=tp_encoder_padding_mask)
            adj_att = F.dropout(adj_att, p=self.dropout, training=self.training)
        ...
        if encoder_padding_mask is not None:
            ...
            mean_mask = mean_mask.unsqueeze(2).repeat(1,1,x.size()[2]).transpose(0, 1).float()
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features(self, prev_output_tokens, encoder_out=None, incremental_state=None, **unused):
        ...
        mean_mask = 1 - target_padding_mask
        mean_mask = mean_mask.unsqueeze(2).repeat(1, 1, x.size()[2]).transpose(0, 1).float()
        target_mean = mean_mask * x
        target_mean = torch.mean(target_mean, dim=0)
        ...
        return x, {'attn': attn, 'inner_states': inner_states, 'target_mean':target_mean, 'tp_mean':encoder_out['tp_mean']}
